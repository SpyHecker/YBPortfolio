...
export function ContactSection({ id = "contact", content, mode }) {
  ...
  const email = contact.email || "yashbharambe@example.com";
  const github = contact.github || "https://github.com/SpyHecker";
  const linkedin = contact.linkedin || "https://linkedin.com/in/yashbharambe";
  const cvPath = contact.cvPath || "/Yash_Bharambe_CV.pdf";
  ...
  const openMailDraft = ({ name, email: senderEmail, message }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setStatus("error");
        setErrorMsg(data.message || "Could not send automatically. Opening your email app instead.");
        openMailDraft(payload);
      }
    } catch {
      setStatus("error");
      setErrorMsg("Network error. Opening your email app instead.");
      openMailDraft(payload);
    }
  };
    ...
    >
      <div className="mx-auto flex max-w-4xl flex-col items-center gap-10 lg:flex-row lg:items-start lg:gap-16">
        ...
        <div className="flex-1 text-center lg:text-left">
          ...
          <button
            onClick={copyEmail}
            className="mt-6 flex items-center gap-2 text-sm transition-colors hover:text-zinc-100 mx-auto lg:mx-0"
            style={{ color: accent }}
          >
          ...
          <div className="mt-4 space-y-3">
            <a href={`mailto:${email}`} className="flex items-center gap-2 text-sm text-zinc-400 transition-colors hover:text-zinc-100 justify-center lg:justify-start">
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 9v.906a2.25 2.25 0 01-1.183 1.981l-6.478 3.488M2.25 9v.906a2.25 2.25 0 001.183 1.981l6.478 3.488m8.839 2.51l-4.66-2.51m0 0l-1.023-.382a2.25 2.25 0 00-1.578 0l-1.023.382m3.624 0l-4.66 2.51M2.25 18V9m19.5 0v9" />
              </svg>
              Send Email Directly
            </a>
            <a href={github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-zinc-400 transition-colors hover:text-zinc-100 justify-center lg:justify-start">
            ...
            <a href={linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-zinc-400 transition-colors hover:text-zinc-100 justify-center lg:justify-start">
              <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-md flex-1 space-y-4">
          <div>
            <label htmlFor="contact-name" className="mb-1.5 block text-[0.62rem] uppercase tracking-[0.2em] text-zinc-500">Name</label>
            <input id="contact-name" name="name" type="text" placeholder="Your name" value={formData.name} onChange={handleChange} className={inputBase} disabled={status === "sending"} />
          </div>
          <div>
            <label htmlFor="contact-email" className="mb-1.5 block text-[0.62rem] uppercase tracking-[0.2em] text-zinc-500">Email</label>
            <input id="contact-email" name="email" type="email" placeholder="you@email.com" value={formData.email} onChange={handleChange} className={inputBase} disabled={status === "sending"} />
          </div>
          <div>
            <label htmlFor="contact-message" className="mb-1.5 block text-[0.62rem] uppercase tracking-[0.2em] text-zinc-500">Description</label>
            <textarea id="contact-message" name="message" rows="5" placeholder="Tell me about your project or say hello..." value={formData.message} onChange={handleChange} className={`${inputBase} resize-none`} disabled={status === "sending"} />
          </div>

          {status === "error" && (
            <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="text-xs text-red-400">✕ {errorMsg}</motion.p>
          )}
          {status === "success" && (
            <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="text-xs" style={{ color: accent }}>✓ Message sent! I&apos;ll get back to you soon.</motion.p>
          )}

          <div className="flex flex-wrap gap-3 pt-2">
            <button type="submit" disabled={status === "sending"} className={`${accentButton} flex items-center gap-2 px-6 py-3 text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-black transition-opacity disabled:opacity-60`}>
              {status === "sending" ? (
                <>
            ...
            <a href={cvPath} download className="flex items-center gap-2 border border-zinc-700 px-6 py-3 text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-zinc-200 transition-colors hover:border-zinc-500">
